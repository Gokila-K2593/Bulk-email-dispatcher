...
function Home() {
    const [emailsInput, setEmailsInput] = (0, react_1.useState)('');
    const [isSubmitting, setIsSubmitting] = (0, react_1.useState)(false);
    const [historyJobs, setHistoryJobs] = (0, react_1.useState)([]);
    const [activeJobId, setActiveJobId] = (0, react_1.useState)(null);
    const [activeJobDetails, setActiveJobDetails] = (0, react_1.useState)(null);
    ...
    const fetchJobStatus = (0, react_1.useCallback)(async (jobId) => {
        try {
    ...
    const updateJobInHistory = (0, react_1.useCallback)((updatedJob) => {
        setHistoryJobs(prev => {
    ...
    // Poll job status
    const startPolling = (0, react_1.useCallback)((jobId) => {
        stopPolling();
      ...
      <aside className="sidebar">
        ...
        <div className="sidebar-history">
          <div className="sidebar-history-title">
            ...
            {historyJobs.length > 0 && (<button onClick={handleClearHistory} style={{
                background: 'none',
                border: 'none',
          ...
          {historyJobs.length > 0 && (<>
              <input type="text" className="search-input" placeholder="Search by Job ID..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}/>
              <div className="filter-group">
                ...
                <button className={`filter-btn ${statusFilter === 'FAILED' ? 'active' : ''}`} onClick={() => setStatusFilter('FAILED')}>
                  Failed
                </button>
              </div>
            </>)}
          ...
          {/* History List */}
          {filteredHistoryJobs.length === 0 ? (<div className="section-empty" style={{ padding: '2rem 1rem' }}>
              ...
              {filteredHistoryJobs.map((job) => {
                ...
                return (<div key={job.id} className={`history-item ${isActive ? 'active' : ''}`} onClick={() => setActiveJobId(job.id)}>
      ...
      <main className="main-workspace">
        ...
        <div className="card">
              ...
              <form onSubmit={handleSubmit}>
                <div className="form-group">
                  ...
                  <textarea className="textarea-input" placeholder="Enter email addresses (separated by commas or newlines)&#13;Example:&#13;john@example.com, jane@example.com&#13;alex@domain.org" value={emailsInput} onChange={(e) => setEmailsInput(e.target.value)} disabled={isSubmitting}/>
                </div>

                {errorMsg && (<div style={{ color: 'var(--status-failed-text)', fontSize: '0.875rem', marginBottom: '1rem', fontWeight: 500 }}>
                    ⚠️ {errorMsg}
                  </div>)}
        ...
        <div className="card">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
                ...
                {activeJobDetails && (<span className={`badge badge-${activeJobDetails.status.toLowerCase()}`}>
                    {activeJobDetails.status}
                  </span>)}
              </div>

              {!activeJobDetails ? (<div className="section-empty">
                  ...
                  <div className="monitor-layout">
                    ...
                    <div className="donut-chart-wrapper">
                      <svg width="120" height="120">
                        ...
                        <circle className="donut-ring" strokeWidth="8" fill="transparent" r={radius} cx="60" cy="60"/>
                        {/* Success slice */}
                        {successFraction > 0 && (<circle className="donut-segment-success" strokeWidth="8" fill="transparent" r={radius} cx="60" cy="60" strokeDasharray={strokeDasharray} strokeDashoffset={successStrokeDashoffset}/>)}
                        {/* Failed slice */}
                        {failedFraction > 0 && (<circle className="donut-segment-failed" strokeWidth="8" fill="transparent" r={radius} cx="60" cy="60" strokeDasharray={strokeDasharray} strokeDashoffset={failedStrokeDashoffset} style={{
                  ...
                  <div className="progress-container">
                    ...
                    <div className="progress-bar-wrapper">
                      <div className="progress-bar-segment progress-bar-success" style={{ width: `${successWidth}%` }}/>
                      <div className="progress-bar-segment progress-bar-failed" style={{ width: `${failedWidth}%` }}/>
                    </div>
                  </div>

                  {activeJobDetails.status === 'PROCESSING' && (<div className="info-box">
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                        <circle cx="12" cy="12" r="10"/>
                  ...
                  <div style={{ marginTop: '2rem', display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                    ...
                    {(activeJobDetails.status === 'COMPLETED' || activeJobDetails.status === 'FAILED') && (<button className="btn" onClick={handleExportCSV} style={{ width: 'auto' }}>
